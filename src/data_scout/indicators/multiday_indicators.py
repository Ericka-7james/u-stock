...
def save_multiday_indicators(
    payload: Dict[str, Any],
    path: Path | str | None = None,
) -> Path:
    """
    Save the multiday indicators payload to disk and return the output path.

    If ``path`` is None, writes to:
        <project_root>/public/data/multiday-indicators.json

    If ``path`` is provided (Path or str), writes there instead.
    """
    if path is None:
        # src/data_scout/indicators/multiday_indicators.py -> project root
        project_root = Path(__file__).resolve().parents[2]
        path_obj = project_root / "public" / "data" / "multiday-indicators.json"
    else:
        path_obj = Path(path)
    ...
    print(f"Saved multiday indicators to {path_obj}")
    return path_obj
