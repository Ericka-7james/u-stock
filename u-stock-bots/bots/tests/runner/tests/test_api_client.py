...
def test_get_status_raises_when_user_id_missing(monkeypatch):
    ...
    assert "Runner missing user_id for get_status" in str(exc.value)
...
def test_submit_intents_raises_when_user_id_missing(monkeypatch):
    monkeypatch.delenv("RUNNER_USER_ID", raising=False)
    monkeypatch.delenv("USTOCK_USER_ID", raising=False)
    ...
    with pytest.raises(RuntimeError) as exc:
        api_client.submit_intents(api, "ema_trend", [])

    assert "Runner missing user_id for submit_intents" in str(exc.value)
...
def test_sync_trade_fills_raises_when_user_id_missing(monkeypatch):
    monkeypatch.delenv("RUNNER_USER_ID", raising=False)
    monkeypatch.delenv("USTOCK_USER_ID", raising=False)
    ...
    with pytest.raises(RuntimeError) as exc:
        api_client.sync_trade_fills(api, bot_id="ema_trend", mode="paper")
